...
const SurveyForm = ({ survey, answers, onAnswer }) => {
  const cols = survey.labels.length;
  return (
    <div className="flex flex-col gap-1">
      ...
      {survey.questions.map((q, qi) => (
        <div key={qi}
          className="grid items-center gap-2 px-4 py-3 rounded-xl transition-all"
          style={{
          ...
          <p className="text-sm" style={{ color: 'var(--cream)' }}>
            <span className="text-xs mr-2 font-mono" style={{ color: 'var(--mist)' }}>{qi + 1}.</span>
            {q}
          </p>
          {survey.labels.map((l, li) => (
            <div key={li} className="flex flex-col items-center gap-1">
              <button
                onClick={() => onAnswer(qi, li + 1)}
        ...
        <div className="max-w-2xl mx-auto">
          <div className="mb-6">
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--mist)' }}>
              Araştırma kapsamında 7 ölçek uygulanmaktadır. Her ölçeği ayrı ayrı doldurabilirsiniz.
            </p>
          </div>
